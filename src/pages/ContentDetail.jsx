        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-4 text-sm text-gray-400">
              <span>Bài học {content.lessonNumber}</span>
              <span>•</span>
              <span>Cập nhật: {content.updatedAt}</span>
              {content.duration && (
                <>
                  <span>•</span>
                  <span>{content.duration} phút đọc</span>
                </>
              )}
            </div>
          </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {content.videos.map((video, index) => (
                    <div
                      key={index}
                      onClick={() =>
                        openMediaModal({ type: "video", ...video })
                      }
                      className="group relative bg-[#0b0f12] border border-[#202934] rounded-xl overflow-hidden cursor-pointer hover:border-purple-500/50 transition"
                    >
                      <div className="relative aspect-video bg-black/30">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center bg-black/40 group-hover:bg-black/60 transition">
                          <div className="w-16 h-16 rounded-full bg-purple-500/90 group-hover:bg-purple-500 flex items-center justify-center transition">
            ...
            {content.images && content.images.length > 0 && (
              <div>
                <h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
                  ...
                  Hình ảnh ({content.images.length})
                </h2>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {content.images.map((image, index) => (
                    <div
                      key={index}
                      onClick={() =>
                        openMediaModal({ type: "image", ...image })
                      }
