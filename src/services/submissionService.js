...
export const getStudentSubmissionDetail = async (submissionId) => {
  try {
    const response = await api.get(`/submissions/${submissionId}`);
    // Response format: { code: "ok", message: "...", data: { ... } }
    // data includes: id, title, statementMd, language, code, status, feedback, suggestion,
    // bigOComplexityTime, bigOComplexitySpace, score, passed, attemptNo, feedbackTeachers, testCases
    return response.data;
  } catch (error) {
